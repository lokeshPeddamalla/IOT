...
stdout, stderr = process.communicate(input='discoverable on'+'\n')
process.wait()
print(f'Bluetooth is discoverable')
...
# Define the UUID for the Serial Port Profile
SPP_UUID = "00001101-0000-1000-8000-00805F9B34DB"
# Create a Bluetooth socket using RFCOMM
server_socket = bluetooth.BluetoothSocket(bluetooth.RFCOMM)

# Bind the socket to any available port on the device
server_socket.bind(("", bluetooth.PORT_ANY))

# Start listening for incoming connections
server_socket.listen(1)

# Advertise the service to clients
...
mobile_ip = []
with open('bluetooth_address.txt', 'r') as file:
...
while True:
    ...
    if addr[0] in mobile_ip:
        ...
        client_socket.close()
        server_socket.close()
        print(f'Trying IP Communication')
