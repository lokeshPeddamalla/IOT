...
with open('bluetooth_address.txt', 'r') as file:
	...
	for l in content:
		l = l.split(' ')
		l = l[-1]
		mobile_addresses.append(l.strip())
...
while True:
	...
	if client_info[0] in mobile_addresses:
		print(f'verifying match with {mobile_addresses}')
		...
		while True:
			try:
				...
				if count < 3:
					...
					client_socket.close()
					print(f'No Acknowledgement Received on Bluetooth switching to AdHoc')
					break
					
	
		client_socket.close()
		print(f'Trying AdHoc Communication')
